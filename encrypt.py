from keygen import generate_round_keys, permute
...
def xor(bits1, bits2):
    "Simple XOR of two bit strings"
    if len(bits1) != len(bits2):
        print("ERROR: XOR inputs not same length")
        return None
    result = ""
    for i in range(len(bits1)):
        result += "0" if bits1[i]==bits2[i] else "1"
    return result
...
def encrypt_block(plaintext64, round_keys):
    ...
    if len(plaintext64) != 64:
        ...
        return None
    # Initial permutation
    permuted = permute(plaintext64, IP)
    L = permuted[:32]
    R = permuted[32:]
